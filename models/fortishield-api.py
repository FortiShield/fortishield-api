...
error_fortishield_package = 0
exception_error = None
try:
    from fortishield import Fortishield
    from fortishield.exception import FortishieldException
    from fortishield.cluster.dapi import dapi
except (ImportError, SyntaxError) as e:
    error = str(e)
    error_fortishield_package = -1
except FortishieldException as e:
    error_fortishield_package = -3
    error = e.message
    error_code = e.code
...
if __name__ == "__main__":
    ...
    if not list_f:
        ...
        if not request:
            print_json("Fortishield-Python Internal Error: Bad JSON input", 1000)
            exit(1)

    if error_fortishield_package < 0:
        if error_fortishield_package == -1:
            print_json("Fortishield-Python Internal Error: {0}".format(error), 1000)
        if error_fortishield_package == -2:
            print_json("Fortishield-Python Internal Error: uncaught exception: {0}".format(exception_error), 1000)
        if error_fortishield_package == -3:
            print_json(error, error_code)
        exit(0)  # error code 0 shows the msg in the API response.

    if 'function' not in request:
        print_json("Fortishield-Python Internal Error: 'JSON input' must have the 'function' key", 1000)
        exit(1)
    ...
    try:
        ...
        print_json("Fortishield-Python Internal Error: {0}".format(str(e)), 1000)
        if debug:
            raise
