...
router.get('/node', cache(), function (req, res) {
    ...
    execute.exec(python_bin, [fortishield_control], data_request, function (data) { res_h.send(req, res, data); });
})
 ...
 *
 * @apiDescription Returns Fortishield statistical information for the current or specified date.
 *
 * @apiExample {curl} Example usage*:
 ...
 *
 *
 * @apiDescription Returns Fortishield statistical information per hour. Each number in the averages field represents the average of alerts per hour.
 *
 * @apiExample {curl} Example usage*:
 ...
 *
 *
 * @apiDescription Returns Fortishield statistical information per week. Each number in the hours field represents the average alerts per hour for that specific day.
 *
 * @apiExample {curl} Example usage*:
...
router.post('/:node_id/files', function(req, res) {
    ...
    if (req.headers['content-type'] == 'application/octet-stream' || req.headers['content-type'] == 'application/xml') {
        ...
        try {
            data_request['arguments']['tmp_file'] = require('../fortishield-api/helpers/files').tmp_file_creator(req.body);
        } catch(err) {
...
/**
 * @api {get} /cluster/configuration/validation Check Fortishield configuration in all cluster nodes
 * @apiName GetClusterConfiguration
 * @apiGroup Files
 *
 * @apiDescription Returns if Fortishield configuration is OK.
 *
 * @apiExample {curl} Example usage:
...
/**
 * @api {get} /cluster/:node_id/configuration/validation Check Fortishield configuration in a cluster node
 ...
 * @apiGroup Configuration
 *
 * @apiParam {String="agent","agentless","analysis","auth","com","csyslog","integrator","logcollector","mail","monitor","request","syscheck","wmodules"} [component] Indicates the fortishield component to check.
