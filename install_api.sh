...
error_and_exit() {
    ...
    if [ -d "/root/fortishield-api-tmp" ]; then
        exec_cmd "rm -rf /root/fortishield-api-tmp"
    fi
...
previous_checks() {
    ...
    if [ "X${arg}" == "Xdependencies" ]; then  # dependencies argument
        ...
        API_SOURCES="/root"
        DOWNLOAD_PATH=$(url_latest_release "fortishield" "fortishield-api")
    else
    ...
    if ! [ -d $FRAMEWORK_PATH ]; then
        print "Can't find $FRAMEWORK_PATH. Is Fortishield installed?.\nExiting."
        exit 1
    fi
    ...
    if [ $NODE_VERSION -lt 4 ]; then
        print "The current version of NodeJS installed is not supported. Fortishield API requires NodeJS 4.x or newer."
...
get_api () {
    if [ "X$DOWNLOAD_PATH" != "X" ]; then
        ...
        if [ -d "$API_SOURCES/fortishield-api-tmp" ]; then
            exec_cmd "rm -rf $API_SOURCES/fortishield-api-tmp"
        fi
        exec_cmd "mkdir -p $API_SOURCES/fortishield-api-tmp"

        exec_cmd "curl -sL $DOWNLOAD_PATH | tar xvz -C $API_SOURCES/fortishield-api-tmp"

        API_SOURCES="$API_SOURCES/fortishield-api-tmp/fortishield-api-*.*"

        API_NEW_VERSION=`cat $API_SOURCES/package.json | grep "version\":" | grep -P "\d+(?:\.\d+){0,2}\-*\w*" -o`
        print "\nInstalling Fortishield API $API_NEW_VERSION"
    else
        API_NEW_VERSION=`cat $API_SOURCES/package.json | grep "version\":" | grep -P "\d+(?:\.\d+){0,2}\-*\w*" -o`
        if [ "X${arg}" == "Xdev" ]; then
            print "\nInstalling Fortishield API $API_NEW_VERSION from current directory [DEV MODE]."
        else
            print "\nInstalling Fortishield API $API_NEW_VERSION from current directory."
        fi
    fi
...
setup_api_permissions () {
    ...
    exec_cmd "chmod ugo-x $API_PATH/scripts/fortishield-api*"
...
setup_api() {
    ...
    if [ -d $API_PATH ]; then
        ...
        if [[ "X${REINSTALL}" != "X" ]]; then
            ...
            while true; do
                read -p "Fortishield API is already installed (version $API_OLD_VERSION). Do you want to update it? [y/n]: " yn
                case $yn in
                    [Yy] ) update="yes"; break;;
                    [Nn] ) update="no"; break;;
                esac
    ...
    if [ "X${arg}" == "Xdev" ]; then
        ...
        # Set up the right permissions for Fortishield API
        setup_api_permissions
